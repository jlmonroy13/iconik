...
export function SalesGoalsOverview({ goals }: SalesGoalsOverviewProps) {
  ...
  const getTrendIcon = (progress: number) => {
    if (progress >= 100) return <TrendingUp className="w-4 h-4 text-green-600" />
    if (progress >= 75) return <TrendingUp className="w-4 h-4 text-yellow-600" />
            ...
            <div key={goal.id} className="space-y-2">
              ...
              <div className="flex items-center justify-between text-xs text-gray-600 dark:text-gray-400">
                <span>
                  ...
                  {' / '}
                  {goal.type === 'REVENUE' ? formatCurrency(goal.targetAmount) : goal.targetAmount.toLocaleString()}
                </span>
                {isOverdue && (
