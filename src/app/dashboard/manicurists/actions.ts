...
export async function createManicurist(data: ManicuristFormData) {
  try {
    ...
    const { selectedServices, schedules, availability, ...manicuristData } = validatedData

    const manicurist = await prisma.manicurist.create({
      data: {
        ...manicuristData,
        spaId,
        schedules: schedules && schedules.length > 0 ? { create: schedules.map(s => ({ ...s, spaId })) } : undefined,
        availability: availability && availability.length > 0 ? { create: availability.map(a => ({ ...a, spaId })) } : undefined,
      },
    })
...
export async function updateManicurist(id: string, data: ManicuristFormData) {
  try {
    ...
    await prisma.manicuristSchedule.deleteMany({ where: { manicuristId: id, spaId } })
    await prisma.manicuristAvailability.deleteMany({ where: { manicuristId: id, spaId } })
