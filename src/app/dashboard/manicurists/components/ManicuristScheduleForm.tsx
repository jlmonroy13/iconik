...
export function ManicuristScheduleForm() {
  ...
  const usedDays = schedules.map((schedule: ManicuristScheduleData) => schedule.dayOfWeek)

  const availableDays = DAYS_OF_WEEK.filter(day => !usedDays.includes(day.value))
  ...
  const updateScheduleField = (index: number, field: keyof ManicuristScheduleData, value: string | number | boolean) => {
    const updatedSchedules = [...schedules]
    updatedSchedules[index] = { ...updatedSchedules[index], [field]: value }
    setValue('schedules', updatedSchedules)
  }

  const getFieldError = (index: number, field: keyof ManicuristScheduleData): string | undefined => {
    const fieldErrors = errors.schedules as { [key: number]: { [key: string]: { message?: string } } } | undefined
