...
export const manicuristAvailabilitySchema = z
  .object({
    date: z.date(),
  ...
  .superRefine((data, ctx) => {
    if (data.isAvailable) {
      ...
      if (
        !data.endTime ||
        !/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(data.endTime)
      ) {
        ctx.addIssue({
          path: ['endTime'],
          code: z.ZodIssueCode.custom,
...
export type ManicuristAvailabilityData = z.infer<
  typeof manicuristAvailabilitySchema
>;
