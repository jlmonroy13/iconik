...
export default function PaymentMethodsClientPage() {
  ...
  const handleCreate = async (data: PaymentMethodFormData) => {
    ...
    if (res.success && res.data) {
      ...
      showError(
        'Error al crear',
        res.message || 'No se pudo crear el método de pago.'
      );
    }
  ...
  const handleEdit = async (data: PaymentMethodFormData) => {
    ...
    if (res.success && res.data) {
      setMethods(prev =>
        prev.map(m =>
          m.id === selectedMethod.id ? normalizeMethod(res.data) : m
        )
      );
      showSuccess(
      ...
      showError(
        'Error al actualizar',
        res.message || 'No se pudo actualizar el método de pago.'
      );
    }
    setIsSubmitting(false);
  };
  ...
  const handleDeleteConfirmed = async () => {
    ...
    if (res.success) {
      ...
      showError(
        'Error al eliminar',
        res.message || 'No se pudo eliminar el método de pago.'
      );
    }
  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        ...
        description={
          selectedMethod
            ? `¿Estás seguro de que deseas eliminar '${selectedMethod.name}'? Esta acción no se puede deshacer.`
            : ''
        }
