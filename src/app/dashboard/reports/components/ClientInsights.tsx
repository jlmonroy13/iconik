      ...
      .map(client => {
        ...
        const lastVisit =
          allServices.length > 0
            ? new Date(
                Math.max(
                  ...allServices.map(s => new Date(s.createdAt).getTime())
                )
              )
            : null;
      ...
      <div className='grid grid-cols-1 sm:grid-cols-3 gap-4'>
        <Card>
          ...
          <CardContent>
            ...
            <p className='text-xs text-gray-500 dark:text-gray-400 mt-1'>
              {clients.length > 0
                ? ((loyaltyStats.vip / clients.length) * 100).toFixed(1)
                : 0}
              % del total
            </p>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className='text-xs text-gray-500 dark:text-gray-400 mt-1'>
              {clients.length > 0
                ? ((loyaltyStats.regular / clients.length) * 100).toFixed(1)
                : 0}
              % del total
            </p>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className='text-xs text-gray-500 dark:text-gray-400 mt-1'>
              {clients.length > 0
                ? ((loyaltyStats.newClients / clients.length) * 100).toFixed(1)
                : 0}
              % del total
            </p>
          </CardContent>
                ...
                <div className='flex items-center gap-3'>
                  ...
                  <div>
                    ...
                    <div className='flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400'>
                      ...
                      <span>•</span>
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs ${getLoyaltyColor(client.loyalty)}`}
                      >
                ...
                {clientStats.map(client => (
                  ...
                  >
                    ...
                    <td className='text-right py-3 px-2'>
                      <span
                        className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-full text-xs ${getLoyaltyColor(client.loyalty)}`}
                      >
                        {getLoyaltyIcon(client.loyalty)}
            ...
            <div className='space-y-4'>
              {clientStats.slice(0, 8).map(client => {
                const maxSpent = Math.max(
                  ...clientStats.map(c => c.totalSpent)
                );
                const percentage =
                  maxSpent > 0 ? (client.totalSpent / maxSpent) * 100 : 0;
              ...
              {clientStats
                ...
                .map(client => {
                  const maxVisits = Math.max(
                    ...clientStats.map(c => c.visitCount)
                  );
                  const percentage =
                    maxVisits > 0 ? (client.visitCount / maxVisits) * 100 : 0;
