...
import { OperatingHoursSchema, DefaultScheduleConfig, type OperatingHoursFormValues } from '../schemas'
import {
  Button,
  Input,
  useNotifications,
...
export function ComplexOperatingHoursForm({ schedules = [] }: ComplexOperatingHoursFormProps) {
  const [isPending, startTransition] = useTransition()
  const { showSuccess, showError } = useNotifications()

  // Initialize with default schedules if none provided
  const defaultSchedules = schedules.length > 0 ? schedules : DefaultScheduleConfig
  ...
  const onSubmit = (data: OperatingHoursFormValues) => {
    startTransition(async () => {
      ...
      if (result.success) {
        showSuccess('Horarios actualizados', 'Los horarios de operación han sido actualizados.')
        form.reset(data)
      } else {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Button
                      type="button"
                      variant={form.watch(`schedules.${index}.isHoliday`) ? 'outline' : 'primary'}
                      size="sm"
                      onClick={() => {
                    ...
                    <Button
                      type="button"
                      variant={form.watch(`schedules.${index}.isHoliday`) ? 'primary' : 'outline'}
                      size="sm"
                      onClick={() => {
                        form.setValue(`schedules.${index}.isHoliday`, true)
                        form.setValue(`schedules.${index}.dayOfWeek`, undefined)
                      }}
                    >
                      Festivo
                    </Button>
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                <div>
                  <p className="font-medium text-gray-900 dark:text-white">
                    {form.watch(`schedules.${index}.isOpen`) ? 'Abierto' : 'Cerrado'}
                  </p>
                ...
                <div className="flex items-center">
                  <input
                    ...
                    onChange={(e) => form.setValue(`schedules.${index}.isOpen`, e.target.checked)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    <Input
                      ...
                      error={form.formState.errors.schedules?.[index]?.startTime?.message}
                      disabled={isPending}
                    />
                  </div>
                  <div className="space-y-2">
                    <Input
                      ...
                      error={form.formState.errors.schedules?.[index]?.endTime?.message}
                      disabled={isPending}
                    />
                  </div>
                  <div className="space-y-2">
                    <Input
                      ...
                      error={form.formState.errors.schedules?.[index]?.description?.message}
                      disabled={isPending}
                    />
                  </div>
                </div>
