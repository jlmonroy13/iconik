...
export function SalesGoalsList({ goals }: SalesGoalsListProps) {
  ...
  const [editingGoal, setEditingGoal] = useState<SalesGoalFormValues & { id?: string } | undefined>(undefined)
  ...
  const handleUpdate = async (data: SalesGoalFormValues) => {
    ...
    if (res.success && res.data) {
      setLocalGoals(localGoals.map(goal => goal.id === editingGoal.id ? res.data : goal))
      showSuccess('Meta actualizada', 'La meta de ventas fue actualizada correctamente.')
    } else {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (res.success) {
      ...
      showSuccess('Meta eliminada', 'La meta de ventas fue eliminada correctamente.')
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="divide-y rounded-lg border overflow-hidden bg-white dark:bg-gray-900">
        {localGoals.length === 0 && (
          <div className="p-4 text-center text-gray-500">No hay metas configuradas.</div>
        )}
        {localGoals.map(goal => {
          const progress = getProgressPercentage(goal.currentAmount, goal.targetAmount)
          const isOverdue = goal.endDate && new Date() > goal.endDate && progress < 100

          return (
            <div key={goal.id} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400 mb-2">
                  <span>{GOAL_TYPE_LABELS[goal.type]} • {PERIOD_LABELS[goal.period]}</span>
                ...
                <div className="flex items-center gap-4 text-sm">
                  <span className="text-gray-600 dark:text-gray-400">
                    Meta: {goal.type === 'REVENUE' ? formatCurrency(goal.targetAmount) : goal.targetAmount.toLocaleString()}
                  </span>
                  <span className="text-gray-600 dark:text-gray-400">
                    Actual: {goal.type === 'REVENUE' ? formatCurrency(goal.currentAmount) : goal.currentAmount.toLocaleString()}
                  </span>
                  <span className={`font-medium ${progress >= 100 ? 'text-green-600' : progress >= 75 ? 'text-yellow-600' : 'text-red-600'}`}>
                    {progress.toFixed(1)}%
                  </span>
                </div>
              ...
              <div className="flex gap-2">
                <Button size="icon" variant="ghost" onClick={() => handleEdit(goal)} disabled={loadingId === goal.id}>
                  <Pencil className="w-4 h-4" />
                </Button>
                <Button size="icon" variant="ghost" onClick={() => handleDelete(goal.id)} disabled={loadingId === goal.id}>
