...
interface ScheduleModalProps {
  ...
  mode: 'create' | 'edit'
  existingSchedules: Array<{ dayOfWeek?: number | null; isHoliday: boolean; id?: string }>
}
...
export function ScheduleModal({ open, onClose, onSubmit, initialData, mode, existingSchedules }: ScheduleModalProps) {
  ...
  const validateDuplicate = (values: ScheduleEntryFormValues) => {
    const isDuplicate = existingSchedules.some(sch => {
      if (mode === 'edit' && initialData?.id && sch.id === initialData.id) return false
      if (values.isHoliday && sch.isHoliday) return true
      if (!values.isHoliday && !sch.isHoliday && sch.dayOfWeek === values.dayOfWeek) return true
      return false
    })
    if (isDuplicate) {
      return { dayOfWeek: { message: 'Ya existe un horario para este día o festivo.' } }
    }
    return {}
  }
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {!form.watch('isHoliday') && (
          <div>
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">Día de la semana</label>
            <select
            ...
            {form.formState.errors.dayOfWeek && (
              <p className="text-xs text-red-600 mt-1">{getError(form.formState.errors.dayOfWeek.message as string | undefined)}</p>
            )}
          </div>
        )}
        {/* Abierto/Cerrado */}
        <div className="flex items-center gap-2">
          <input
            ...
            id="isOpen"
          />
          <label htmlFor="isOpen" className="text-sm font-medium text-gray-700 dark:text-gray-300">
        ...
        {form.watch('isOpen') && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input
              ...
              error={getError(form.formState.errors.startTime?.message as string | undefined)}
            ...
            <Input
              ...
              error={getError(form.formState.errors.endTime?.message as string | undefined)}
              disabled={form.formState.isSubmitting}
            />
          </div>
        )}
        ...
        <Input
          ...
          error={getError(form.formState.errors.description?.message as string | undefined)}
          disabled={form.formState.isSubmitting}
        />
