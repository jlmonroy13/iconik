...
export function calculateTotal<T extends { amount: number }>(
  items: T[]
): number {
...
export function calculateEfficiencyScore(
  ...
  return Math.round(
    (completionRate * 0.4 + onTimeRate * 0.4 + qualityScore * 20) / 2
  );
}
...
export function calculateRevenuePerService(
  totalRevenue: number,
...
export function getDashboardInsights(stats: DashboardStats): Array<{
  ...
  const avgRevenuePerService = calculateRevenuePerService(
    stats.monthRevenue,
    stats.monthServices
  );
...
export function validateDashboardData(stats: DashboardStats): {
  ...
  if (stats.todayAppointments > 100)
    errors.push('Unrealistic number of appointments today');
  if (stats.monthRevenue > 100000000)
    errors.push('Unrealistic monthly revenue');
