...
import { Input, Button, useNotifications, Card, CardContent } from "@/components/ui"
    ...
    const fetchSpas = async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.error('❌ Error fetching spas:', response.status, response.statusText, errorData)
        }
      } catch (error) {
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-white text-2xl font-bold">
                    {spas.reduce((total, spa) => total + (spa._count?.users || 0), 0)}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="p-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-white text-2xl font-bold">
                    {spas.reduce((total, spa) => total + (spa._count?.appointments || 0), 0)}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="p-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-white text-2xl font-bold">
                    {spas.reduce((total, spa) => total + (spa._count?.services || 0), 0)}
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          {spas.length === 0 ? (
            <Card className="bg-white/10 border-white/20">
              <CardContent className="p-8 text-center">
                ...
                <h3 className="text-xl font-semibold text-white mb-2">No hay spas creados</h3>
                <p className="text-gray-300 mb-4">Crea el primer spa para comenzar</p>
                <Button
                ...
                >
                  <CardContent className="p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        <h3 className="text-xl font-semibold text-white mb-1">{spa.name}</h3>
                        <p className="text-gray-300 text-sm">/{spa.slug}</p>
                      </div>
                      <div className={`w-3 h-3 rounded-full ${spa.isActive ? 'bg-green-400' : 'bg-red-400'}`} />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div className="text-center">
                        ...
                        <p className="text-white font-semibold">{spa._count?.users || 0}</p>
                      </div>
                      <div className="text-center">
                        <p className="text-gray-300">Clientes</p>
                        <p className="text-white font-semibold">{spa._count?.clients || 0}</p>
                      </div>
                      <div className="text-center">
                        <p className="text-gray-300">Citas</p>
                        <p className="text-white font-semibold">{spa._count?.appointments || 0}</p>
                      </div>
                      <div className="text-center">
                        <p className="text-gray-300">Servicios</p>
                        <p className="text-white font-semibold">{spa._count?.services || 0}</p>
                      </div>
                    </div>
        ...
        {showCreateForm && (
          <Card className="bg-white/10 border-white/20">
            <CardContent className="p-8">
              <h2 className="text-2xl font-bold text-white mb-6">Crear Nuevo Spa</h2>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h3 className="text-lg font-semibold mb-4 text-white">Información del Spa</h3>
                    <Input
                  ...
                  <div>
                    <h3 className="text-lg font-semibold mb-4 text-white">Administrador del Spa</h3>
                    <Input
