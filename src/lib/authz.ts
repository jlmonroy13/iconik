 ...
 */
export function canManageAppointments(user: User, spaId: string, branchId?: string): boolean {
  if (user.role === 'SUPER_ADMIN') return true
  if (user.role === 'SPA_ADMIN' && user.spaId === spaId) return true
  if (user.role === 'BRANCH_ADMIN' && user.spaId === spaId && (!branchId || user.branchId === branchId)) return true
  return false
}
 ...
 */
export function canViewReports(user: User, spaId: string, branchId?: string): boolean {
  if (user.role === 'SUPER_ADMIN') return true
  if (user.role === 'SPA_ADMIN' && user.spaId === spaId) return true
  if (user.role === 'BRANCH_ADMIN' && user.spaId === spaId && (!branchId || user.branchId === branchId)) return true
  return false
...
export function canManageUsers(user: User, targetSpaId?: string): boolean {
  if (user.role === 'SUPER_ADMIN') return true
  if (user.role === 'SPA_ADMIN' && (!targetSpaId || user.spaId === targetSpaId)) return true
  return false
}
 ...
 */
export function getUserAccessLevel(user: User): 'system' | 'spa' | 'branch' | 'individual' {
  switch (user.role) {
    case 'SUPER_ADMIN':
      return 'system'
    case 'SPA_ADMIN':
 ...
 */
export function canAccessFeature(user: User, feature: string, spaId?: string, branchId?: string): boolean {
  switch (feature) {
    case 'spa-management':
