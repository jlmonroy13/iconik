...
export function testEnums() {
  ...
  const appointmentStatuses = ['PENDING_APPROVAL', 'SCHEDULED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED', 'NO_SHOW']
  console.log('✅ AppointmentStatus enum values:', appointmentStatuses)

  // Test ServiceType enum
  const serviceTypes = ['MANICURE', 'PEDICURE', 'NAIL_ART', 'GEL_POLISH', 'ACRYLIC_NAILS', 'NAIL_REPAIR', 'HAND_SPA', 'FOOT_SPA', 'OTHER']
  console.log('✅ ServiceType enum values:', serviceTypes)

  // Test CashRegisterTransactionType enum
  const transactionTypes = ['PAYMENT_RECEIVED', 'PAYMENT_REFUND', 'CASH_DEPOSIT', 'CASH_WITHDRAWAL', 'EXPENSE_PAYMENT', 'CHANGE_GIVEN', 'OPENING_BALANCE', 'CLOSING_BALANCE', 'ADJUSTMENT', 'OTHER']
