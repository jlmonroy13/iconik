 ...
 */
export function calculateMonthlyExpenseAmount(amount: number, frequency: string): number {
  switch (frequency) {
    case 'DAILY':
      return amount * 30
    case 'WEEKLY':
 ...
 */
export function calculateAnnualExpenseAmount(amount: number, frequency: string): number {
  switch (frequency) {
    case 'DAILY':
      return amount * 365
    case 'WEEKLY':
 ...
 */
export function getExpenseStatus(dueDate: Date, isPaid: boolean): {
 ...
 */
export function calculateExpenseSummary(expenses: Array<{
  amount: number
  type: string
  isPaid: boolean
  frequency: string
  ...
  expenses.forEach(expense => {
    const monthlyAmount = calculateMonthlyExpenseAmount(expense.amount, expense.frequency)
